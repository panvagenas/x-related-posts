...
class tracker extends framework
{
    ...
    public function setAsVisited($pid)
    {
        ...
        $visited[$pid] = $pid;
        $this->©cookie->set($this->getCookieName(), serialize($visited), 86400);
    }

    public function getVisited()
    {
        $c = $this->©cookie->get($this->getCookieName());
        return $c ? unserialize($c) : array();
    }

    public function getCookieName()
    {
        $name = md5($this->instance->plugin_root_ns);
        return $name;
    }

    public function maybeTrackLink($link)
    {
        if (!$this->isEnabled()) {
            return $link;
        }
        ...
        return $link.(is_numeric(strpos($link, '?')) ? '&xrp_f='.$post->ID : '?xrp_f='.$post->ID);
    }
